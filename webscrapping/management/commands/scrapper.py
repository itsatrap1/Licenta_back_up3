            ...
            try:
                ...
                for k in range(len(lift_number_web_elements)):
                    ...
                    if lift_number_web_elements[k].text != '' and lift_name_web_elements[k].text != ' ':
                        lift_names_and_numbers[i].append(
                            [lift_number_web_elements[k].text, lift_name_web_elements[k].text])
        ...
        for i in range(len(full_dictionary['Name'])):
            ...
            try:
                Resorts.objects.create(
                    ...
                    circulating_ropeway_gondola_lift = full_dictionary['Circulating ropeway/gondola lift'][i],
                    chairlift = full_dictionary['Chairlift'][i],
                    tbar_lift = full_dictionary['T-bar lift/platter/button lift'][i],
                    rope_tow_baby_lift = full_dictionary['Rope tow/baby lift'][i],
                    aerial_tramway =  full_dictionary['Aerial tramway/reversible ropeway'][i],
                    moving_carpet = full_dictionary['People mover/Moving Carpet'][i],
                    combined_installation_gondola_chair = full_dictionary['Combined installation (gondola and chair)'][i],
                    funicular = full_dictionary['Funicular'][i],
                    cog_railway = full_dictionary['Cog railway'][i]
                    )
                print('%s added' % (name,))
            except:
